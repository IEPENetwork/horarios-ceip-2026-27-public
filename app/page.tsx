...
function openStage(stage: Stage) { const url = new URL(window.location.href); url.searchParams.set("etapa", stage); window.location.assign(url); }
...
function timeBounds(value: string) { return value.split("–").map((part) => { const [hours, minutes] = part.split(":").map(Number); return hours * 60 + minutes; }); }
function lessonsInSlot(group: string, day: string, slot: string) { const [slotStart, slotEnd] = timeBounds(slot); return lessonsFor(group, day).filter((lesson) => { const [start, end] = timeBounds(lesson.time); return start >= slotStart && end <= slotEnd; }).sort((a, b) => timeBounds(a.time)[0] - timeBounds(b.time)[0]); }
...
export default function Home() {
  if (!HAS_SELECTED_STAGE) return <StageLanding/>;
...
function LessonCard({ lesson, query, compact = false, date }: { lesson: Lesson; query: string; compact?: boolean; date?: string }) {
  ...
  return <article className={`${compact ? "matrix-card" : "lesson"} family-${family(lesson.subject)} ${visible ? "" : "muted"} ${absent.length ? "has-absence" : ""}`}><span className="lesson-time">{lesson.time}</span><strong>{lesson.subject}</strong><small><b>Docente:</b> {lesson.primaryDisplay || lesson.primary}</small>{lesson.shared.length > 0 && <small><b>{lesson.supportLabel || "Docencia compartida"}:</b> {lesson.shared.map((teacher) => lesson.sharedDisplay?.[teacher] || teacher).join(", ")}</small>}{absent.length > 0 && <div className="absence-stack">{absent.map((teacher) => { const substitute = savedSubstitution(records, date!, lesson, teacher); return <span className="absence-badge" key={teacher}>Ausente: {teacher}{substitute ? ` · Sustituye: ${substitute}` : " · Pendiente"}</span>; })}</div>}</article>;
}

function WeekGrid({ group, query }: { group: string; query: string }) {
  const { referenceDate } = useAbsenceContext();
  return <div className="week-grid">{DAYS.map((day) => <div className="day" key={day}><h3>{day}</h3><div className="day-cards">{schedule.slots[day as keyof typeof schedule.slots].map((time) => { if (isRecess(day, time)) return <div className="break" key={time}><span>☕</span> Recreo · 30 min</div>; return lessonsInSlot(group, day, time).map((lesson) => <LessonCard key={lesson.time} lesson={lesson} query={query} date={dateForDay(referenceDate, day)}/>); })}</div></div>)}</div>;
}
...
function DayView({ day, setDay, query, setQuery }: { day: string; setDay: (day: string) => void; query: string; setQuery: (value: string) => void }) {
  ...
  const gridStyle = { "--slot-count": slots.length, gridTemplateColumns: `minmax(78px, .45fr) repeat(${slots.length}, minmax(0, 1fr))` } as React.CSSProperties;
  return <><Toolbar query={query} setQuery={setQuery}><label>Día<select value={day} onChange={(event) => setDay(event.target.value)}>{DAYS.map((name) => <option key={name}>{name}</option>)}</select></label><PrintOptions buttonLabel="Imprimir día" fitScale={.92}/></Toolbar><section className="panel day-overview"><PanelTitle eyebrow="Organización diaria" title={day}/><div className="matrix-wrap"><div className="day-matrix" style={gridStyle}><div className="matrix-heading">Grupo</div>{slots.map((time) => <div className="matrix-heading" key={time}>{time}</div>)}{GROUPS.map((group) => <div className="matrix-row" key={group}><div className="matrix-group">{group}</div>{slots.map((time) => { if (isRecess(day, time)) return <div className="matrix-recess" key={time}>☕ Recreo</div>; const lessons = lessonsInSlot(group, day, time); return <div className="matrix-cell" key={time}>{lessons.length ? lessons.map((lesson) => <LessonCard key={lesson.time} lesson={lesson} query={query} compact date={dateForDay(referenceDate, day)}/>) : <span>—</span>}</div>; })}</div>)}</div></div></section></>;
}
...
function SubstitutionsView() {
  ...
  return <><div className="sub-tabs"><button className={view === "resolver" ? "active" : ""} onClick={() => setView("resolver")}>Resolver faltas</button><button className={view === "availability" ? "active" : ""} onClick={() => setView("availability")}>Disponibilidad por franja</button><button className={view === "support" ? "active" : ""} onClick={() => setView("support")}>Apoyos disponibles</button><button className={view === "dashboard" ? "active" : ""} onClick={() => setView("dashboard")}>Dashboard e historial</button></div>{view !== "dashboard" && <div className="toolbar standalone-print"><PrintOptions buttonLabel="Imprimir vista de sustituciones" fitScale={.9}/></div>}
...
function PrintCenter() {
  ...
  const automaticScale = paper === "A4" ? (orientation === "landscape" ? .98 : .82) : 1;
  const effectiveScale = fitMode === "fit" ? automaticScale : Number(scale) / 100;
...
function PrintDayReport({ day }: { day: string }) {
  ...
  return <section className="print-sheet print-sheet-day"><PrintSheetHeader eyebrow="Organización diaria" title={day}/><div className="day-matrix print-day-matrix" style={{ gridTemplateColumns: `90px repeat(${slots.length}, minmax(0, 1fr))` }}><div className="matrix-heading">Grupo</div>{slots.map((time) => <div className="matrix-heading" key={time}>{time}</div>)}{GROUPS.map((group) => <div className="matrix-row" key={group}><div className="matrix-group">{group}</div>{slots.map((time) => { if (isRecess(day, time)) return <div className="matrix-recess" key={time}>☕ Recreo</div>; const lessons = lessonsInSlot(group, day, time); return <div className="matrix-cell" key={time}>{lessons.length ? lessons.map((lesson) => <LessonCard key={lesson.time} lesson={lesson} query="" compact date={dateForDay(referenceDate, day)}/>) : <span>—</span>}</div>; })}</div>)}</div></section>;
}
